...
> 文档同步日期：2026-08-26。双图形后端、16 槽相机资源调度、华睿相机内存保护、任务独立输入、PLC 单槽握手、视频 YOLO、多点找色快速路径、启动诊断和统一发布包均已按当前代码复核。当前构建、回归和未闭环验收见 [`docs/STATUS_2026-08-26.md`](docs/STATUS_2026-08-26.md)，全部文档入口见 `docs/README.md`。
...
| 启动诊断 | 正式包由 `Windows_imgui.exe` 检查同目录核心程序和运行库；Visual Studio 直接调试 Bootstrap 时自动从父级构建目录启动主程序，不再误报整包文件缺失 |
...
| 手动执行输入 | 任务只有明确绑定相机槽时才使用对应相机帧；未绑定任务始终使用任务文件夹、任务单图或公共图片，混合执行不会串图 |
| 多相机 | 16 个相机槽分别维护连接、重连、采集线程、触发和最新帧；多槽执行等待各绑定槽本轮新帧并按任务分发，且不切换用户当前预览 |
| 相机资源调度 | 相机按需激活和逐槽取帧，限制同时执行任务数；华睿帧通过同步 `IMV_GetFrame` 获取，在 SDK 缓冲有效期内复制后始终配对 `IMV_ReleaseFrame`，避免多相机累计占用导致闪退 |
| PLC 触发输入 | 在线相机 → 任务文件夹 → 任务单图 → 公共图片；在线相机本轮抓帧失败会明确输出 Error |
| 执行方式 | 支持执行全部、执行当前任务、全部单步、当前任务单步、循环和最多 4 个任务并行；待执行任务只保存共享帧引用，进入 worker 后才创建隔离副本 |
...
| 视频检测与高 DPI | 单 YOLO 可在视频播放期间发布异步检测结果；连续检测使用“开始实时检测”；窗口、字体和停靠布局适配 Per-Monitor V2 与 150% 高分屏 |
| 正式发布 | 对外交付只使用 `dist/IMgui_Opencv-Release-x64.zip`，包内包含 YOLO、OCR 模型；不再保留日期版或 `hidpi` 变体 |

历史合并说明见 `docs/STATUS_2026-07-25.md`，任务分组收尾见 `docs/STATUS_2026-07-26.md`，PLC 工业握手初版见 `docs/STATUS_2026-07-27.md`，工业功能整合见 `docs/STATUS_2026-07-28.md`，双图形后端状态见 `docs/STATUS_2026-07-30.md`，界面和发布资源整理见 `docs/STATUS_2026-08-02.md`，工具链统一检查见 `docs/STATUS_2026-08-05.md`；当前总状态见 `docs/STATUS_2026-08-26.md`。
...
│   ├── ToolRunPolicy.h/cpp     ←   执行顺序、异步工具和跨任务依赖策略
│   ├── TaskImageProvider.h/cpp ←   UTF-8 任务图片读取与文件夹轮换
...
│   ├── ToolsWindow.cpp/h       ←   工具参数与执行入口
│   ├── TaskGroupWindow.cpp/h   ←   任务列表、工具分配、输入绑定和筛选状态
...
├── App/                        ← Win32 应用壳、入口与资源
│   ├── Main.cpp                ←   程序入口 + 主循环
│   ├── AppIncludes.h           ←   入口专用头文件汇总
│   └── Platform.h              ←   Win32 / DirectX / OpenCV 平台头
├── README.md                   ← 本文件
...
| 主题切换 | 现代深色/明亮模式，统一面板层级、控件状态与原生标题栏，并自动持久化 |
...
任务输入按以下规则解析：任务明确绑定相机槽且相机已连接时，先请求对应槽位的一帧；相机不可用或抓帧失败时使用该任务绑定的文件夹或单图；任务未绑定相机时不会读取任何在线相机，按任务文件夹、单图或当前公共图片运行。文件夹在每轮开始时推进一张，同一任务内所有工具使用同一张本轮图片。完整说明见 `docs/TASK_GROUPS.md`。
...
`ToolsWindow.cpp` 已将流程图、任务组管理和全部工具参数面板迁出；当前主要架构债集中在卡片通用外壳、部分兼容引用，以及 `ToolExecutor` 中仍按 type 维护的参数同步。
