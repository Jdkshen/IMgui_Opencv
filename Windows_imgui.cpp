					  ...
					  _In_ int nCmdShow)
{
	...
	WNDCLASSEXW wc = {sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, appInstance, appIcon, nullptr, nullptr, nullptr, L"ImGui Example", appIconSmall};
	::RegisterClassExW(&wc);
	...
	// 日志输出到 exe 所在目录（而非工作目录）
	static char s_LogPath[MAX_PATH];
	GetModuleFileNameA(nullptr, s_LogPath, MAX_PATH);
	char *lastSlash = strrchr(s_LogPath, '\\');
	if (lastSlash)
		*(lastSlash + 1) = '\0';
	strcat_s(s_LogPath, "imgui_log.txt");
	io.LogFilename = s_LogPath;
	...
	LogSystem::Add(LOG_INFO, "APP started");
