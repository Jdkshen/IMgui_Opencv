...
const char *g_ThemeNames[] = {"现代深色", "明亮"};
...
namespace
{
    ...
    std::filesystem::path ThemeConfigPath()
    {
        ...
        if (length == 0 || length >= std::size(executablePath))
            return L"theme.cfg";
        return std::filesystem::path(executablePath).parent_path() / L"theme.cfg";
    }
    ...
    void ApplyNativeTitleBarTheme(int theme)
    {
        ...
        // Keep the native non-client area close to ImGui's menu/title layer.
        // A #3c3c3c caption creates a detached light strip above this compact
        // shell, so use the active-title surface (#1f1f1f) instead.
        const COLORREF caption = dark ? RGB(31, 31, 31) : RGB(236, 240, 243);
        const COLORREF text = dark ? RGB(204, 204, 204) : RGB(26, 33, 41);
        const COLORREF border = dark ? RGB(43, 43, 43) : RGB(184, 194, 201);
        ...
        // Force Windows to repaint the non-client frame immediately when the
        // user changes themes; otherwise the old caption can remain until the
        // window is resized or loses focus.
...
void ApplyTheme(int theme)
{
    ...
    // Neutral IDE-style surface hierarchy with restrained cyan interaction accents.
    if (theme == 0)
    {
        ImGui::StyleColorsDark();
        ImVec4 *c = style.Colors;
        c[ImGuiCol_Text]                  = ImVec4(0.80f, 0.80f, 0.80f, 1.00f);
        c[ImGuiCol_TextDisabled]          = ImVec4(0.52f, 0.52f, 0.52f, 1.00f);
        c[ImGuiCol_WindowBg]              = ImVec4(0.118f, 0.118f, 0.118f, 1.00f); // #1e1e1e
        c[ImGuiCol_ChildBg]               = ImVec4(0.145f, 0.145f, 0.149f, 1.00f); // #252526
        c[ImGuiCol_PopupBg]               = ImVec4(0.145f, 0.145f, 0.149f, 0.99f);
        c[ImGuiCol_Border]                = ImVec4(0.169f, 0.169f, 0.169f, 1.00f); // #2b2b2b
        c[ImGuiCol_BorderShadow]          = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
        c[ImGuiCol_FrameBg]               = ImVec4(0.176f, 0.176f, 0.188f, 1.00f); // #2d2d30
        c[ImGuiCol_FrameBgHovered]        = ImVec4(0.235f, 0.235f, 0.235f, 1.00f); // #3c3c3c
        c[ImGuiCol_FrameBgActive]         = ImVec4(0.14f, 0.31f, 0.38f, 1.00f);
        c[ImGuiCol_TitleBg]               = ImVec4(0.094f, 0.094f, 0.094f, 1.00f); // #181818
        c[ImGuiCol_TitleBgActive]         = ImVec4(0.122f, 0.122f, 0.122f, 1.00f); // #1f1f1f
        c[ImGuiCol_TitleBgCollapsed]      = ImVec4(0.094f, 0.094f, 0.094f, 1.00f);
        c[ImGuiCol_MenuBarBg]             = ImVec4(0.094f, 0.094f, 0.094f, 1.00f);
        ...
        c[ImGuiCol_Button]                = ImVec4(0.176f, 0.176f, 0.188f, 1.00f); // #2d2d30
        c[ImGuiCol_ButtonHovered]         = ImVec4(0.235f, 0.235f, 0.235f, 1.00f);
        c[ImGuiCol_ButtonActive]          = ImVec4(0.15f, 0.43f, 0.52f, 1.00f);
        c[ImGuiCol_Header]                = ImVec4(0.176f, 0.176f, 0.188f, 1.00f);
        c[ImGuiCol_HeaderHovered]         = ImVec4(0.216f, 0.216f, 0.239f, 1.00f);
        ...
        c[ImGuiCol_Tab]                   = ImVec4(0.094f, 0.094f, 0.094f, 1.00f);
        c[ImGuiCol_TabHovered]            = ImVec4(0.165f, 0.176f, 0.180f, 1.00f);
        c[ImGuiCol_TabSelected]           = ImVec4(0.122f, 0.122f, 0.122f, 1.00f);
        c[ImGuiCol_TabSelectedOverline]   = ImVec4(0.29f, 0.73f, 0.82f, 1.00f);
        c[ImGuiCol_TabDimmed]             = ImVec4(0.094f, 0.094f, 0.094f, 1.00f);
        c[ImGuiCol_TabDimmedSelected]     = ImVec4(0.122f, 0.122f, 0.122f, 1.00f);
        ...
        c[ImGuiCol_TableHeaderBg]         = ImVec4(0.145f, 0.145f, 0.149f, 1.00f);
        c[ImGuiCol_TableBorderStrong]     = ImVec4(0.235f, 0.235f, 0.235f, 1.00f);
        c[ImGuiCol_TableBorderLight]      = ImVec4(0.169f, 0.169f, 0.169f, 1.00f);
        c[ImGuiCol_TableRowBgAlt]         = ImVec4(0.145f, 0.145f, 0.149f, 0.55f);
        c[ImGuiCol_TextSelectedBg]        = ImVec4(0.20f, 0.53f, 0.62f, 0.45f);
        c[ImGuiCol_TextLink]              = ImVec4(0.34f, 0.72f, 0.88f, 1.00f);
        c[ImGuiCol_ResizeGrip]            = ImVec4(0.24f, 0.56f, 0.66f, 0.18f);
        c[ImGuiCol_ResizeGripHovered]     = ImVec4(0.29f, 0.70f, 0.80f, 0.55f);
        c[ImGuiCol_ResizeGripActive]      = ImVec4(0.32f, 0.78f, 0.86f, 0.90f);
        c[ImGuiCol_DragDropTarget]        = ImVec4(0.32f, 0.78f, 0.86f, 0.95f);
        c[ImGuiCol_DragDropTargetBg]      = ImVec4(0.20f, 0.53f, 0.62f, 0.18f);
        c[ImGuiCol_ModalWindowDimBg]      = ImVec4(0.02f, 0.025f, 0.03f, 0.72f);
    ...
    else
    {
        ...
        c[ImGuiCol_ScrollbarBg]           = ImVec4(0.89f, 0.905f, 0.915f, 0.00f);
    ...
    // 始终写到 EXE 同目录，避免从 VS、快捷方式或资源管理器启动时
    // 分别生成多份 theme.cfg，造成菜单状态与实际颜色不一致。
