...
#include <vector>
#include <string>
struct ShapeMatch { cv::Rect bbox; double score=0,shapeScore=0; float angle=0; int tmplIdx=-1; std::vector<cv::Point> points; double area=0; bool isGreen=false; };
namespace ShapeMatcher {
    struct Params { int blurSize=5,tplRetrMode=0; double tplMinArea=30,minScore=0.5,minShapeScore=0.3; int lineThickness=2,maxResults=1; bool showLabels=true;
        ...
        bool enableRotation=false; int rotationStart=-45,rotationEnd=45,rotationStep=5;
        // 模板预处理
