...
namespace TemplateMatch
{
    ...
    void Run()
    {
        ...
        snprintf(dbg, sizeof(dbg), "[TM] DONE: %d results | match=%.3fms NMS=%.3fms total=%.3fms\n",
            (int)gMatchROIs.size(), timeMatch, timeNms, g_TMLastMatchTime);
        OutputDebugStringA(dbg);

        LogSystem::Add(LOG_INFO, color,
            "模板匹配: %d个区域 | 匹配=%.3fms NMS=%.3fms 总=%.3fms",
            (int)gMatchROIs.size(), timeMatch, timeNms, g_TMLastMatchTime);
        // 调试日志: 每个匹配结果
        for (int i = 0; i < (int)gMatchROIs.size() && i < 20; i++) {
            ...
            float angle = (i < (int)gMatchAngles.size()) ? gMatchAngles[i] : 0.0f;
            LogSystem::Add(LOG_INFO, ImVec4(0.3f, 0.9f, 0.3f, 1),
                "  #%d: s=%.4f [%.0f,%.0f %.0fx%.0f] ang=%.1f",
                i+1, score, rx, ry, rw, rh, angle);
        }
        if ((int)gMatchROIs.size() > 20)
            LogSystem::Add(LOG_INFO, color, "  ... (还有 %d 个)", (int)gMatchROIs.size() - 20);
    }
    ...
    void ShowTemplateEditor()
    {
        ...
        if (ImGui::Begin("模板编辑", &g_ShowTplEditor))
        {
            ...
            // 处理后的模板像素渲染
            {
                ...
                cv::Mat preview;
                cv::resize(tpl, preview, cv::Size(dw, dh), 0, 0, cv::INTER_NEAREST);

                ImDrawList* dl = ImGui::GetWindowDrawList();
                ImVec2 base = ImGui::GetCursorScreenPos();
                float step = 2.5f;
                bool isColor = !g_TplGray && !g_TplBinary && !g_TplEdge && preview.channels() >= 3;
                for (int y = 0; y < dh; y++)
                {
                    for (int x = 0; x < dw; x++)
                    {
                        ...
                        if (isColor)
                        {
                            auto& px = preview.at<cv::Vec3b>(y, x);
                            col = IM_COL32(px[2], px[1], px[0], 255);
                        }
                        else
                        {
                            uchar v = (preview.channels() == 1) ? preview.at<uchar>(y, x)
                                : (uchar)(preview.at<cv::Vec3b>(y, x)[0] * 0.3f + preview.at<cv::Vec3b>(y, x)[1] * 0.59f + preview.at<cv::Vec3b>(y, x)[2] * 0.11f);
    ...
    void ShowWindow()
    {
        ...
        if (ImGui::Begin("模板匹配调试", &g_ShowTemplateMatch))  //,ImGuiWindowFlags_NoDocking  禁止停靠窗口
        {
            ...
            if (g_ShowPreview && !g_FrozenTemplate.empty())
                {
                    ...
                    int dw = (int)(tpl.cols * rs), dh = (int)(tpl.rows * rs);
                    if (dw < 2) dw = 2; if (dh < 2) dh = 2;
                    cv::Mat preview; cv::resize(tpl, preview, cv::Size(dw, dh), 0, 0, cv::INTER_NEAREST);
                    ImDrawList* dl = ImGui::GetWindowDrawList();
            ...
            ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1),
                "预估耗时 ~%.3fms | 缩放到 %dpx 以内", estTime, g_TMMaxImageDim);
