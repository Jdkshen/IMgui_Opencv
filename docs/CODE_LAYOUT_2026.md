...
> 文档同步日期：2026-07-27。当前边界已包含任务分组、独立输入、并行执行、PLC IO 映射和握手回归。
...
当前工具范围是 type `0` 到 `17`，type `12` 是原图链路节点。`all_tools_test.recipe` 仍保留 version 2，用于验证当前 version 4 加载器的向后兼容；YOLO 条目需要有效的本地模型路径才能通过推理。
