...
> 文档复核日期：2026-08-26。当前适配器已覆盖多槽按需连接、同步取帧的所有权复制、队列释放和内存异常诊断；运行库部署规则保持不变。

应用已接入华睿/iRAYPLE 普通 GigE、USB 工业相机 SDK（`IMV_*` 接口）。IP 相机应选择“华睿 iRAYPLE”后端，不要选择采集卡的 `IMV_FG_*` 接口。

## 多相机内存规则

- 每次同步 `IMV_GetFrame` 成功后，在 SDK 缓冲有效期内完成图像数据复制，并在所有成功、校验失败、转换失败和内存异常路径配对 `IMV_ReleaseFrame`；应用不保留 `IMV_Frame` 中的临时指针。
- 断开、停止采集和重连会清空旧帧与待处理请求，避免第二台及后续相机连接时累计上一槽的 SDK 缓冲。
- 任务执行继续使用 `cv::Mat` 引用计数和写时复制；只有算法确实修改输入时才创建独立图像。
- 正式 ZIP 只有在打包机找到完整 MV Viewer `Runtime/x64` 目录及 VC++ 2013 x64 Runtime 时才携带华睿 SDK；否则目标机必须安装 MV Viewer Runtime。
