...
> 文档同步日期：2026-08-25。本文说明当前双图形后端构建、字体/图标/模型资源打包、启动诊断、CI、PLC 联调和唯一正式发布包规则。
...
当前标准 ZIP 的构建与验收状态见 [STATUS_2026-08-26.md](STATUS_2026-08-26.md)。打包脚本会同时生成同名 `.zip.sha256` 文件，上传 GitHub Release 时必须与 ZIP 一并上传。若主程序仍在运行，Windows 会锁定 EXE 或运行时文件；应关闭程序后重新构建和打包，不再通过日期后缀、`hidpi` 或 `_updated` 文件名规避文件锁。

发布包的 `Windows_imgui.exe` 是静态启动诊断器，实际核心为同目录 `Windows_imgui_core.exe`。Visual Studio 调试 Bootstrap 时，启动器识别 `x64/<Configuration>/bootstrap` 开发布局并从父目录启动主程序；该兼容逻辑不会放宽正式包的相邻文件检查。

截至 2026-08-25，GitHub 仓库尚无 Release 或版本标签；首次发布建议使用 `v1.0.0`，须在源码提交与运行包内容一致后创建。
