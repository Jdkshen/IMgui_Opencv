...
> 文档同步日期：2026-08-25。任务级输入、按相机槽取帧、并行执行、视频单 YOLO、PLC 指定任务入口、ToolResult 来源/耗时、18 类工具上下游能力和配方 version 5 已复核。

结果卡片的任务耗时为该任务工具 `prepare + execute` 之和；顶部“本轮总耗时”是整轮墙钟时间。并行任务中的共享模型推理段带互斥保护，因此相机取帧和非模型工具可并行，单 Session YOLO 推理仍按安全顺序执行。
