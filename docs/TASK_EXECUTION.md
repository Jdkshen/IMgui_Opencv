...
> 文档同步日期：2026-08-26。本文中的“任务”主要指开发工作项；产品内任务分组见 [TASK_GROUPS.md](TASK_GROUPS.md)，多相机见 [MULTI_CAMERA.md](MULTI_CAMERA.md)，PLC 时序见 [HARDWARE_INTEGRATION.md](HARDWARE_INTEGRATION.md)。
...
| P0-3 | 完整回归基线 | ✅ 回归入口统一显式关闭工具控制器和硬件运行时；`--task-images-only`、`--plc-handshake-only` 和 Release 全量回归均通过，退出码 0 | `Test/RegressionTests.vcxproj`、`Test/regression_tests.cpp` |
...
| P3-2 | 多任务配方 | ✅ 最多 16 个任务；任务级单图/文件夹、绑定相机槽、全部/当前任务/单步、最多 4 任务并行及任务结果总览均已接入；待执行队列不再提前深拷贝整图，输入副本峰值受 worker 上限约束；完整回归状态以最新状态页为准 | `Core/ToolChainState.*`、`Core/ToolController.*`、`UI/ToolsWindow.cpp`、`UI/RunResultWindow.cpp` |
| P4-1 | 工程发布 | ✅ 主程序和回归工程 Release x64 构建通过，全量回归已恢复；唯一运行包仍按发布脚本校验，真实设备与长稳测试保留为现场门槛 | `.github/workflows/`、`scripts/`、`docs/RELEASE.md` |
