...
> 文档同步日期：2026-08-25。本文以当前 type 0-17、`VisionContext`、`ITool`、`ToolResult` 和任务执行链为准；18 类工具的输入、ROI、上下游、结果能力、Fixture、统一显示和性能计时已按当前代码复核。

多点找色使用共享输入和局部灰度/二值临时图；普通轴对齐矩形 ROI 直接在原图坐标内扫描，不再先提取旋转副本。YOLO 的卡片耗时是工具 `prepare + execute`，同图同模型缓存命中仍包含整图哈希；不同 ROI、置信度或图像会分别触发真实推理。
