...
| 项 | 状态 |
| --- | --- |
| 工具数量 | 13 个 ITool 工具 + 1 个原图特殊工具 |
| ITool 工具 | 13 个：type 0-11、13（边缘、模板、Blob、阈值、YOLO、轮廓、形状、直线、形态学、颜色、多点找色、YOLO OpenCV 5.0、OCR文字识别） |
...
| 配方 | 支持工具实例序列化，包含 `inputSourceMode` 和 OCR 模型/字典参数 |
| 结果导出 | `Core/ResultExporter` 支持 JSON 结果、PNG 结果截图、Markdown 运行报告 |
| OCR | type 13，PP-OCRv6 tiny + NCNN，默认模型路径 `models\ppocrv6\*` |
| 最近编译 | 待重新确认；上次已确认 Debug/Release 可生成产物，当前新增 OCR/导出后应重新编译 |
| 下一批规划 | 配方加载后自动执行；随后尺寸测量、Blob 增强、二维码/条码、图像差分；OCR 后续优化 |
