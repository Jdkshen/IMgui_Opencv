...
> 文档同步日期：2026-08-25。工具数量、任务分组、16 槽相机资源调度、视频检测、DPI、UI 布局/资源、统一结果、启动诊断、发布命名、PLC IO 映射和握手维护规则已同步。
...
- Bootstrap 的 Visual Studio 开发布局和正式发布包布局是否都通过 `--diagnostic-only`，并能启动各自对应的主程序。
...
| 最近验证 | 2026-08-25 Release x64 主程序和回归工程已生成；多点找色、任务图像与完整回归通过，Bootstrap 开发/发布布局通过；当前总状态见 `STATUS_2026-08-25.md` |
