...
> 文档同步日期：2026-08-16。工具数量、任务分组、16 槽相机、视频检测、DPI、UI 布局/资源、统一结果、发布命名、PLC IO 映射和握手维护规则已同步。
...
| 最近验证 | Release x64 主程序和回归工程已生成；`--examples-only` 已通过，完整回归仍在任务文件夹输入回归结束阶段触发 `0xC0000409`，不得写成全部通过；当前总状态见 `STATUS_2026-08-16.md` |
